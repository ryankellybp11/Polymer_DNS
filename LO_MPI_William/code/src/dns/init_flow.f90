...
if (flow_select .le. 0 .or. flow_select .eq. 5) then
   ...
   call vortex_new(initu,initv,initw)
...
subroutine vortex_new(initu,initv,initw)
...
! sufficient solution)
    ...
    do i = 0,vNum-1
        ...
        do j = 1,nz
            ...
            do k = 1,nyp
                ...
                vortZ = z - vzc
                vortR2 = (vortY)**2 + (vortZ)**2
                if (vortR2 .ne. 0.0) then
                    vortWi(j,k) = vortWi(j,k) - vSign*sinh(2*pi/zl*vortY)/(cosh(2*pi/zl*vortY) - cos(2*pi/zl*vortZ))*(1.0 - exp(-vortR2/vortSigma**2))
                    vortVi(j,k) = vortVi(j,k) + vSign*sin(2*pi/zl*vortZ)/(cosh(2*pi/zl*vortY) - cos(2*pi/zl*vortZ))*(1.0 - exp(-vortR2/vortSigma**2))
                end if
    ...
    do k = 1, nyp
        do j = 1,nz
            do i = 1,nx
                initu(k,j,i) = 0.0
                initv(k,j,i) =  vortGamma/(2.0*zl)*vortVi(j,k)
                initw(k,j,i) = -vortGamma/(2.0*zl)*vortWi(j,k)
            end do
        end do
    end do
