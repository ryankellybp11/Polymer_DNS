subroutine part_track(u,v,w,omx,omy,omz,u_old,v_old,w_old, & 
                      ...
                      Lu,Lv,Lw,Lu_old,Lv_old,Lw_old,area,rank)
...
! ============================================================================ !
    ...
    call swirl_count(xpart,ypart,zpart,swirl_part,u11,u12,u13,u21,u22,u23,u31,u32,u33,area,rank)
...
!    call swirl_trap_y(ypart,swirl_part)
    ...
    if ((mod(it,iprnfrq) .eq. 0 .or. it .eq. 1) .and. it .ne. 0) then
        if (npart .le. 10000) then ! write particle data to ASCII file
            ...
            write(directory_part,'("outputs",i2.2,"/particles/")') rank
...
subroutine swirl_count(xpart,ypart,zpart,swirl_part,u11,u12,u13,u21,u22,u23,u31,u32,u33,area,rank)
...
! particles inside a vortex using a given threshold value
    ...
    if (it .eq. 1) then
        open(30, file = dirname//"swirl10_norm")
        open(31, file = dirname//"pArea")
        open(32, file = dirname//"vArea")
        write(30,*) (pArea-vArea)/vArea*100
        write(31,*) pArea
        write(32,*) vArea
    else
        open(30, file = dirname//"swirl10_norm", position = "append")
        open(31, file = dirname//"pArea", position = "append")
        open(32, file = dirname//"vArea", position = "append")
        write(30,*) (pArea-vArea)/vArea*100
        write(31,*) pArea
        write(32,*) vArea
    end if
...
#ifdef OUTPUTFORM
subroutine write_particles_szplt(xp,yp,zp,up,vp,wp,swirl,rank)
    ...
    ! Open files
    write(filename,'("outputs",i2.2,"/particles/part-t-",i6.6,".szplt")') rank,it
