...
! ====================================================================== !
    ...
    real wrk1(nyp,nz,nx),wrk2(nyp,nz,nx),wrk3(nyp,nz,nx)
...
! Done adding common blocks (I regrouped some, but that may change as I see how they're used)
    ...
    character(33) :: filename1,filename2,filename3
  !............................................................................
...
!       temperature conditions on the bottom (RAH  7/11/96)
      ...
      ib = 0
      att = 0.0 ! zero temp on top 1/11/10
      btt = 1.0
      abb = 0.0
      bbb = 1.0 ! zero temp on bot 7/12/07 rah 
...
!  changed to only have one subforce routine - Ryan 8/11/23
       if (ipolyflag .eq. 1 .and. it .ge. (src_start)) then
            ...
            ! Print out polymer force terms for post-processing analysis
            if ((mod(it,iprnfrq) .eq. 0 .and. it .ne. 0) .or. it .eq. 1) then
                call ccopy (nz*nxh*nyp,t1,1,wrkc,1)
                call xyzfft(wrkc,w1,w2,wfft1,wfft2,wfft3,wfft4,1)
                call unscram(wrkc,wrk1)
                call ccopy (nz*nxh*nyp,t2,1,wrkc,1)
                call xyzfft(wrkc,w1,w2,wfft1,wfft2,wfft3,wfft4,1)
                call unscram(wrkc,wrk2)
                call ccopy (nz*nxh*nyp,t3,1,wrkc,1)
                call xyzfft(wrkc,w1,w2,wfft1,wfft2,wfft3,wfft4,1)
                call unscram(wrkc,wrk3)
    
                write(filename1,'("outputs/polyforce/fp_x-",i6.6,".dat")')it
                write(filename2,'("outputs/polyforce/fp_y-",i6.6,".dat")')it
                write(filename3,'("outputs/polyforce/fp_z-",i6.6,".dat")')it
                open(61,file=filename1)
                open(62,file=filename2)
                open(63,file=filename3)
       ...
       do  j = 1,nz       
          ...
          do  i = 1,nx
            ...
            ! Note (k,j,i) order!
             ...
             c33(k,j,i) = c33z
             if (scl_flag .eq. 1) then
                 sc(k,j,i) = deltat*exp(-(betay + betaz))
             else if (scl_flag .eq. 2) then
...
!    if (it .lt. src_stop .and. it .ge. src_start + 100) then
    if (scl_flag .eq. 2 .and. it .lt. src_stop .and. it .ge. src_start) then
...
! name - Ryan 9/23/22
    ...
    if (scl_flag .ge. 1) then
        ...
        i = tecini142("Solution", "swirl u v w wx wy wz ctp scalar", filename//NULLCHR, ".", &
