...
! ============================================================================ !
    ...
    if (scl_flag .eq. 1) then
        do k = 1,nx
            xcor = xl*(float(k-1)/float(nx)) - xl/2.0
            xsq  = (xcor - xc1)**2
            betax = xsq/(2.0*sigmax**2)
            if (betax .lt. 18.0) then
            do j = 1,nz
                zcor = zl*(float(j-1)/float(nz)) - zl/2.0
                zsq  = (zcor - zc1)**2
        ...
        do k = 1,nx
            do j = 1,nz
                ...
                do i = 1,nyp
                    ...
                    ! Normalize by area
                    Aring = pi*(vortR + 3.0*sigmay)**2 - pi*(vortR - 3.0*sigmay)**2

                    ! Initialize Scalar
                    scl(i,j,k) = deltaT*exp(-(rcor - vortR)**2/(2.0*sigmay**2))/Aring
                end do
            end do
        end do
    end if
