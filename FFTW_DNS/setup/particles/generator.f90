program generator
    ...
    if (new_part .eq. 11) then
    ...
    else if (new_part .eq. 23) then
        ! Uniform distrubtion on a given x-plane assumes npart is a 5th power,
        ! e.g., 4^5 = 1024
        npz = 128 !(npart/2)**(3./5.)
        npy = 128 !2*(npart/2)**(2./5.)
        ...
        do k = 1,npz
            do i = 1,npy
                yp = float(i-1)*(yl/float(npy)) + (yl/float(npy))/2.0
                zp = float(k-1)*(zl/float(npz)) + (zl/float(npz))/2.0
        ...
        do k = 1,npx
            do i = 1,npy
                yp = float(i-1)*(yl/float(npy)) + (yl/float(npy))/2.0
                xp = float(k-1)*(xl/float(npx)) + (xl/float(npx))/2.0
