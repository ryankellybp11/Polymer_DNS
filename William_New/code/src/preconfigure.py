...
if new_part > 0:
        ...
        if (ipoly != 0):
                ...
                print('    4. Compiling DNS code with scalar...')
        else:
                print('    4. Compiling DNS code (fluid only)... ')
else:
        if (ipoly != 0):
                print('    3. Compiling DNS code with polymer... ')
        elif (iscl != 0):
                print('    3. Compiling DNS code with scalar...')
        else:
                print('    3. Compiling DNS code (fluid only)... ')


#compiler_options = ['-mavx', '-r8', '-fr', '-132', '-w', '-O2', '-mcmodel', 'medium', '-fpp', '-shared-intel','-g','-traceback','-Wall','-check all']
compiler_options = ['-mavx', '-r8', '-fr', '-132', '-w', '-O3', '-unroll', '-qopenmp', '-parallel', '-mcmodel', 'medium', '-fpp', '-shared-intel','-g','-traceback','-Wall','-check all']

if (ipoly != 0):
    # DNS w/ Polymer
    source_files = ['../src/dns/grid_size.f90', '../src/dns/derivatives.f90', '../src/dns/helpers.f90', '../src/dns/solvers.f90', '../src/dns/polymer_dns.f90', '../src/dns/ffts.f90', '../src/dns/init_flow.f90', '../src/dns/part_track.f90']
elif (iscl != 0):
    # DNS w/ Scalar
    source_files = ['../src/dns/grid_size.f90', '../src/dns/derivatives.f90', '../src/dns/helpers.f90', '../src/dns/solvers.f90', '../src/dns/scalar_dns.f90', '../src/dns/ffts.f90', '../src/dns/init_flow.f90', '../src/dns/part_track.f90']
else:    
    # DNS fluid only
    source_files = ['../src/dns/grid_size.f90', '../src/dns/derivatives.f90', '../src/dns/helpers.f90', '../src/dns/solvers.f90', '../src/dns/newtonian_dns.f90', '../src/dns/ffts.f90', '../src/dns/init_flow.f90', '../src/dns/part_track.f90']
